...
def csv():
	# dir = '/data/Bingli/NAC_MRI/CLAM-master/heatmaps/demo/slides'
	dir = '/media/data/Bingli/NAC_MRI/WSI/wai/zeng'
	save_dir = '/media/data/Bingli/NAC_MRI/CLAM-master/heatmaps/process_lists'
	clipath = '/media/data/Bingli/NAC_MRI/CLAM_data'
	cli_df = pd.read_csv(os.path.join(clipath, 'pcr_vs_no+qz+zeng.csv'), encoding='utf=8', index_col=0)
	result = []
	result2 = []
	...
	for i in range(0,len(lists)):
		...
		if 'csv' in d:
			continue
		if int(d) in cli_df.index:
			label = cli_df[cli_df.index == int(d)]['label'].iloc[0]
			path = os.listdir(os.path.join(dir, d))[0].split('.')[0]
			name = d + '/'+path
			result = np.array([name,label])
			result2.append(result)
			# result = pd.DataFrame(result)
			# result_t = result.transpose()

			# result2.append(result_t)
	result2 = pd.DataFrame(result2)
	result2.to_csv(os.path.join(save_dir,'data_qz2.csv'))
...
if __name__ == '__main__':
	...
	for i in range(len(process_stack)):
		...
		r_slide_save_dir = os.path.join(exp_args.raw_save_dir, exp_args.save_exp_code, str(grouping))
		...
		if isinstance(data_args.data_dir, str):
			...
			raise NotImplementedError
		slide_id0 = slide_id.split('/')[0]
		r_slide_save_dir0 = os.path.join(r_slide_save_dir, slide_id0)
		os.makedirs(r_slide_save_dir0, exist_ok=True)
		...
		if vis_params['vis_level'] < 0:
			best_level = wsi_object.wsi.get_best_level_for_downsample(256)## 5
			# best_level = wsi_object.wsi.get_best_level_for_downsample(32)## 5
			vis_params['vis_level'] = best_level
		mask = wsi_object.visWSI(**vis_params, number_contours=True) #334*710
		mask.save(mask_path)
		...
		for sample in samples:
			if sample['sample']:
				...
				for idx, (s_coord, s_score) in enumerate(zip(sample_results['sampled_coords'], sample_results['sampled_scores'])):
					...
					slide_id0 = slide_id.split('/')[0]
					slide_id1 = slide_id.split('/')[1]
					sample_save_dir1 = os.path.join(sample_save_dir,slide_id0)
					os.makedirs(sample_save_dir1, exist_ok=True)
					# patch.save(os.path.join(sample_save_dir, '{}_{}_x_{}_y_{}_a_{:.3f}.png'.format(slide_id0,idx, slide_id, s_coord[0], s_coord[1], s_score)))
					patch.save(os.path.join(sample_save_dir1, '{}_{}_x_{}_y_{}_a_{:.3f}.png'.format(idx, slide_id1, s_coord[0], s_coord[1], s_score)))
		...
		if os.path.isfile(os.path.join(p_slide_save_dir, heatmap_save_name)):
			...
			heatmap_save_name0 = heatmap_save_name.split('/')[0]
			heatmap_save_name1 = heatmap_save_name.split('/')[1]
			p_slide_save_dir1 = os.path.join(p_slide_save_dir, heatmap_save_name0)
			os.makedirs(p_slide_save_dir1, exist_ok=True)

			if heatmap_args.save_ext == 'jpg':
				heatmap.save(os.path.join(p_slide_save_dir1, heatmap_save_name1), quality=100)
			else:
				heatmap.save(os.path.join(p_slide_save_dir1, heatmap_save_name1))
		...
		if heatmap_args.save_orig:
			...
			heatmap_save_name = '{}_orig_{}.{}'.format(slide_id1,int(vis_level), heatmap_args.save_ext)
			p_slide_save_dir1 = os.path.join(p_slide_save_dir, slide_id0)
			if os.path.isfile(os.path.join(p_slide_save_dir, heatmap_save_name)):
				pass
			else:
				heatmap = wsi_object.visWSI(vis_level=vis_level, view_slide_only=True, custom_downsample=heatmap_args.custom_downsample)
				if heatmap_args.save_ext == 'jpg':
					heatmap.save(os.path.join(p_slide_save_dir1, heatmap_save_name), quality=100)
				else:
					heatmap.save(os.path.join(p_slide_save_dir1, heatmap_save_name))
